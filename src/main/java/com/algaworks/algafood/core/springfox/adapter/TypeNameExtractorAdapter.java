...
public class TypeNameExtractorAdapter extends TypeNameExtractor {
    ...
    public TypeNameExtractorAdapter(TypeResolver typeResolver, PluginRegistry<TypeNameProviderPlugin, DocumentationType> typeNameProviders, EnumTypeDeterminer enumTypeDeterminer, TypeResolver typeResolver1, PluginRegistry<TypeNameProviderPlugin, DocumentationType> typeNameProviders1, EnumTypeDeterminer enumTypeDeterminer1) {
        super(typeResolver, typeNameProviders, enumTypeDeterminer);

        this.typeResolver = typeResolver1;
        this.typeNameProviders = typeNameProviders1;
    ...
    private String innerTypeName(ResolvedType type, ModelContext context) {
        return type.getTypeParameters().size() > 0 && type.getErasedType()
                .getTypeParameters().length > 0 ? this.genericTypeName(type, context) : this.simpleTypeName(type, context);
    }
